...
export const runDailyPipingReport = async () => {
    try {
        console.log("[PipingReport] Iniciando reporte diario...");
        ...
        const ejecutadasHoy = ejecuciones.filter((e: any) => {
            ...
            // Manejar tanto formato ISO (2026-03-25) como local (25/03/2026) dependendiendo de la API
        ...
        const spoolsHoy = spools.filter((s: any) => {
            ...
            const rawHoy = today.toISOString().split('T')[0]; // asumiendo YYYY-MM-DD en bbdd
        ...
        const message = `📊 *Andina PRY-413 | Resumen ${weekStr}*\n` +
            `📅 Fecha: ${strHoy}\n` +
            `━━━━━━━━━━━━━━━━━━━━\n` +
        ...
        for (const chatId of recipients) {
            try {
                await bot.api.sendMessage(chatId, message, { parse_mode: 'Markdown', disable_web_page_preview: true });
            } catch (err: any) {
